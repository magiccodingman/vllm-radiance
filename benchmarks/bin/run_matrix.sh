...
for config in "${selected[@]}"; do
  case "$config" in
    tp2_spec-off)         tp=2; spec=off; util=${TP2_GPU_UTIL:-0.90}; max_len=${TP2_MAX_MODEL_LEN:-32768}; eager=0 ;;
    tp1-eager8k_spec-off) tp=1; spec=off; util=${TP1_GPU_UTIL:-0.95}; max_len=${TP1_MAX_MODEL_LEN:-8192}; eager=1 ;;
    tp2_spec-on)          tp=2; spec=on;  util=${TP2_GPU_UTIL:-0.90}; max_len=${TP2_MAX_MODEL_LEN:-32768}; eager=0 ;;
