"""Replay a frozen Pi corpus through compiled M1 and optimized M8, with checkpoints.
...
def worker(args):
    ...
    config = make_config(spec, "fixed-bf16", speculation=args.arm == "m8")
    ...
    with reject_dead_native_rpcs(engine.engine_core):
        ...
        require(
            ...
            "actual backend is not compiled",
        )
        require("PIECEWISE" in metadata["graph_mode"], "GPU graphs were not enabled")
        require(
        ...
        for index, row in enumerate(corpus["continuations"]):
            ...
            request = f"compiled-corpus-{args.arm}-{index}"
            ...
            seal(
                {
                    ...
                    "decode": aggregate(all_rows),
                    "prefill": aggregate(all_prefills),
                    "scope": "compiled GPU-graph forced-token M1/M8; full BF16 target head; "
...
def run(args):
        ...
        seal(
            {
                ...
                **manifests,
                ...
                "reference": "fresh compiled M1; eager rows are not reused",
            }
        ),
    )
    with gpu_lease(args.output / "gpu-lease"):
        for arm in ("m1", "m8"):
            ...
            env.update(
                {
                    ...
                    "QWEN_OPTIMIZED_REPAIR": str(args.repair_manifest),
                    "QWEN_OPTIMIZED_PERFORMANCE": str(args.performance_manifest),
            ...
            require(code == 0, "compiled replay failed; private log retained")
    write_private(
